...
import { saveAgenda, completeTrip } from "../../ducks/userReducer";
...
class Home extends Component {
    ...
    handleAgenda(agendaNameInput, destinationInput, activityInput, budgetInput, notesInput, time) {
        let { currentAgenda, newDay, currentDay } = this.state;
        if (newDay) {
            this.props.saveAgenda(newDay, currentDay, currentAgenda, agendaNameInput, destinationInput, activityInput, budgetInput, notesInput, time);
            this.setState({
                newDay: false
            })
        } else {
    ...
    render() {
        const { day, agenda, currentDay, currentAgenda, time, agendaNameInput, destinationInput, activityInput, budgetInput, notesInput, nextStepsFlag } = this.state;
                    ...
                    <div className="home-wrapper" onClick={() => this.handleHamburgerMenu()} >
                        ...
                        <div className="home-container-wrapper">
                            <div className="home-container">
                                <input className="home-name-input home-inputs" type="text" placeholder="Agenda Name" value={agendaNameInput} onChange={e => this.handleInput('agendaNameInput', e.target.value)} />
                                <div className="home-inputs-container">
                                    ...
                                    <div className="home-time-agenda-container">
                                        ...
                                        <button className="home-save-agenda-btn" onClick={() => this.handleAgenda(agendaNameInput, destinationInput, activityInput, budgetInput, notesInput, time)}>Add Agenda</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {nextStepsFlag ? (
                            <div>
                                <button onClick={() => this.props.completeTrip(days) }>Complete Trip</button>
...
export default connect(mapStateToProps, { toggleHamburgerBtn, saveAgenda, completeTrip })(Home);
